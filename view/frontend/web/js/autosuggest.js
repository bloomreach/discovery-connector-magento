...
function findUpElementByTagName(startElement,tagName){let element=startElement
...
;return autosuggestResultsContainerElement}function injectAutosuggestElements(){
var _a;const searchInput=getAutosuggestSearchInputElement()
...
}))}let scheduleSubmission=false;const formFocusListener=()=>{
scheduleSubmission=true};const formBlurListener=()=>{scheduleSubmission=false}
;const formElementSubmitListener=()=>{var _a,_b,_c;scheduleSubmission=false
;const searchData={
...
};const windowUnloadListener=()=>{if(scheduleSubmission){
formElementSubmitListener()}};function addFormElementSubmitListener(){
const suggestElement=getAutosuggestSearchInputElement()
;if(!suggestElement)return
;const element=findUpElementByTagName(suggestElement,"form");if(!element)return
...
capture:true});window.addEventListener("beforeunload",windowUnloadListener,true)
...
var debounce_1=debounce;const searchInputElementBlurListener=()=>{
if(getCurrentAutosuggestUiState().mouseDownEventHappenedInsideAutosuggestResultsContainer){
updateCurrentAutosuggestUiState({
mouseDownEventHappenedInsideAutosuggestResultsContainer:false});return false}
const containerElement=getAutosuggestResultsContainerElement()
;if(!containerElement)return;containerElement.innerHTML="";return true}
...
updateCurrentAutosuggestUiState({
mouseDownEventHappenedInsideAutosuggestResultsContainer:true})}else{
const containerElement=getAutosuggestResultsContainerElement()
...
;getCurrentAutosuggestRequestState();if(lastTemplateData){
const containerElement=getAutosuggestResultsContainerElement()
...
;const element=getAutosuggestSearchInputElement();if(!element)return
...
}function addSearchInputElementListeners(){addSearchInputElementBlurListener()
;addSearchInputElementFocusListener();addSearchInputElementChangeListener()}
...
}));return processedResults}const log=Debug("br:autosuggest")
;function buildAutosuggestModule(){log("Build Autosuggest Module:","4.0.0")
;let currentAutosuggestRequestState={request_id:0,last_template_data:null,
currentElement:null,pollIntervalId:null};let currentAutosuggestUiState={
mouseDownEventHappenedInsideAutosuggestResultsContainer:false};return{
...
setCurrentAutosuggestUiState:uiState=>{currentAutosuggestUiState=uiState},
getCurrentAutosuggestUiState:()=>currentAutosuggestUiState,load:async()=>{
initPolling()},unload:()=>{var _a
;const state=getCurrentAutosuggestRequestState()
;window.clearInterval((_a=state.pollIntervalId)!==null&&_a!==void 0?_a:void 0)
;moduleWillUnmount()}}}function initPolling(){var _a
;let state=getCurrentAutosuggestRequestState()
;window.clearInterval((_a=state.pollIntervalId)!==null&&_a!==void 0?_a:void 0)
;const doPoll=()=>{state=getCurrentAutosuggestRequestState()
;const currentElement=state.currentElement
;const foundElement=getAutosuggestSearchInputElement()
;if(currentElement&&!foundElement){updateCurrentAutosuggestRequestState({
currentElement:null});moduleWillUnmount()
}else if(!currentElement&&foundElement){updateCurrentAutosuggestRequestState({
currentElement:foundElement});moduleWillMount()}}
;if(!state.pollIntervalId)log("Polling initialized")
;const pollTimer=window.setInterval(doPoll,100)
;updateCurrentAutosuggestRequestState({pollIntervalId:pollTimer});doPoll()}
function moduleWillUnmount(){log("Unmounting Module")
;removeAutosuggestElements()}function moduleWillMount(){var _a,_b
;log("Mounting Module");try{injectAutosuggestElements()}catch(err){
log((err===null||err===void 0?void 0:err.stack)||(err===null||err===void 0?void 0:err.message))
}try{areRequirementsMet()}catch(err){log("Requirements not met, aborting")
;moduleWillUnmount();return}addSearchInputElementListeners()
...
;log("Fetching suggestions for",query);const config=buildAutosuggestConfig()
...
;log.verbose("Using config",config);log.verbose("Render to:",container)
...
function buildApiCallParameters(query){var _a,_b,_c
;const config=buildAutosuggestConfig()
;const urlParameters=new URLSearchParams(window.location.search)
...
}function areRequirementsMet(){var _a;const config=buildAutosuggestConfig()
;const check={account_id:config.account_id,domain_key:config.domain_key,
...
;invariant(check.domain_key,"domain_key must be set");return check.enabled}
Debug("br:product-events");Debug("br:recommendations");debounce_1((event=>{
var _a
...
isCategoryPage:false})}const autosuggestModule=buildAutosuggestModule()
;window.BloomreachModules=Object.assign(Object.assign({},globalBloomreachModules),{
autosuggest:autosuggestModule});autosuggestModule.load().catch(console.error)
})();
