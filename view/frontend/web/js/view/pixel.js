...
define([
  ...
  // pixel has loaded.
  window.pixelReady =
    window.pixelReady || new Promise((r) => (window.pixelResolve = r));
    ...
    initBaseEvents: async function () {
      ...
      const getAttributeId = function (dataSelector) {
        try {
          ...
          const hasIndexAccessor =
            superAttr.includes("[") && superAttr.includes("]");
      ...
      const getAttributesMetrics = function (jq) {
        ...
        if (!addToCartContainer) {
          console.warn(
            "Could not determine product container of the Add to Cart button."
          );
          return out;
        }
      ...
      const handleClickAddToCart = function (event) {
        ...
        // fails.
        addToCartJq.attr(
          sdkConstants.ADD_TO_CART_ATTRIBUTE_DISABLE,
          `${!allowEvent}`
        );
        ...
        if (!productInfo) {
          console.warn(
            "Bloomreach Connector: Unable to determine product information for pixel event"
          );
        ...
        if (productInfo.childAttributeMap && allowEvent) {
          br_data.sku = getSimpleProductSkuFromAttributes(
            productInfo.childAttributeMap,
            getAttributesMetrics(addToCartJq)
          );
          addToCartJq.attr(sdkConstants.ADD_TO_CART_ATTRIBUTE_SKU, br_data.sku);
          addToCartJq.attr(
            sdkConstants.ADD_TO_CART_ATTRIBUTE_PROD_ID,
            productInfo.sku
          );
        }
      ...
      if (needsProductSkuInfo.length > 0) {
        fetch(`${BASE_URL}rest/V1/bloomreach/product/skus`, {
          method: "POST",
          headers: {
