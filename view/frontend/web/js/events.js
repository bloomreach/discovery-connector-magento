...
;return autosuggestResultsContainerElement}function getAddToCartElements(){
...
}));return processedResults}const log$1=Debug("br:autosuggest")
...
}function addToCartElementClickListener(e){var _a,_b
...
;const eventProperties={prod_id:blmAddToCartProdId,sku:blmAddToCartSku}
...
const elements=getAddToCartElements();elements.forEach((element=>{
...
}))}function quickviewElementClickListener(e){var _a,_b
;const quickviewElement=e.currentTarget;quickviewElement.dataset
...
prod_id:blmQuickviewProdId,prod_name:blmQuickviewProdName,sku:blmQuickviewSku})}
function addQuickviewElementClickListener(){
const elements=getQuickviewElements();elements.forEach((quickviewElement=>{
...
}))}const log=Debug("br:product-events");let pollTimer=void 0;const current={
addToCart:[],quickview:[]};function buildProductEventsModule(){return{
load:async()=>{initPolling()}}}function initPolling(){
window.clearInterval(pollTimer);const doPoll=()=>{const found={addToCart:[],
quickview:[]};found.addToCart=getAddToCartElements()
;found.quickview=getQuickviewElements()
;if(current.addToCart.length&&!found.addToCart.length){current.addToCart=[]
;addToCartWillUnmount()
}else if(!current.addToCart.length&&found.addToCart.length){
current.addToCart=found.addToCart;addToCartWillMount()}
if(current.quickview.length&&!found.quickview.length){current.quickview=[]
;quickviewWillUnmount()
}else if(!current.quickview.length&&found.quickview.length){
current.quickview=found.quickview;quickviewWillMount()}}
;if(!pollTimer)log("Polling initialized")
;pollTimer=window.setInterval(doPoll,100);doPoll()}
function addToCartWillMount(){log("Add to cart events will mount")
;addAddToCartElementClickListener()}function addToCartWillUnmount(){
log("Add to cart events will unmount")}function quickviewWillMount(){
log("Quickview events will mount");addQuickviewElementClickListener()}
function quickviewWillUnmount(){log("Quickview events will unmount")}
Debug("br:recommendations");debounce_1((event=>{var _a
...
isCategoryPage:false})}const productEventsModule=buildProductEventsModule()
;window.BloomreachModules=Object.assign(Object.assign({},globalBloomreachModules),{
events:productEventsModule});productEventsModule.load().catch(console.error)
})();
