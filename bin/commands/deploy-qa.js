...
async function updateRemote(deployInfo) {
  ...
  if (shell.exec(`ssh ${deployInfo.host} "cd /var/www/html; bin/magento setup:upgrade; bin/magento cache:flush"`).code !== 0) {
    console.error("Was not able to upgrade the remote magento server to reflect the deploy");
  }
}
...
async function sync(deployInfo) {
  console.log("Syncing files", deployInfo.src, `${deployInfo.host}:${deployInfo.dest}`);
  let resolve;
...
async function run({ src, dest }) {
  ...
  const questions = [
    deployInfo.host ? (Array.isArray(deployInfo.host) ? {
      type: 'autocomplete',
      name: 'host',
    ...
    deployInfo.src ? void 0 : {
      type: 'input',
      name: 'src',
      message: 'What src folder should be uploaded?'
    },
    deployInfo.dest ? void 0 : {
      type: 'input',
      name: 'dest',
  ...
  deployInfo.src = path.resolve(deployInfo.src) + (src.endsWith('/') ? '/' : '');
